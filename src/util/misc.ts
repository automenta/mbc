...
export const formatTimestamp = (ts: number) =>
  defaultDateFormatter.format(new Date(ts * 1000))
...
export const formatTimestampAsDate = (ts: number) =>
  dateFormatterAsDate.format(new Date(ts * 1000))
...
export const createScroller = (
  ...
  const startObserver = (targetElement: Element) => {
    observer = new IntersectionObserver(
      async (entries, observerInstance) => { // Renamed observer to observerInstance to avoid shadowing
...
export const tryFetch = <T>(f: () => Promise<T>) => // Assuming f is async
  tryCatch(f, (e: Error) => {
...
const toSnakeCase = (str: string) => // Renamed to be more descriptive and follow convention
  str.replace(/[-_ ]+/g, "_")
...
export const race = <T>(threshold: number, promises: Promise<T>[]) => { // Added generic type
  let count = 0

  if (threshold <= 0) { // Corrected condition to handle 0 and negative thresholds
    return Promise.resolve()
  }

  return new Promise<void>((resolve, reject) => {
    if (promises.length === 0) { // Handle empty promises array to prevent infinite loop
      resolve()
      return
    }
    promises.forEach(p => {
...
export const displayDomain = (url: string) =>
  first(displayUrl(url).split(/[\/\?]/)) || '' // Added default in case of empty split
...
export const updateIn = <T, K extends keyof T>(k: K, f: (x: T[K]) => T[K]) => // Improved type safety with generics and keyof
  (x: T) => ({...x, [k]: f(x[k])})
