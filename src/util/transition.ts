...
export function slideAndFade(
  ...
  const borderWidthStartValue = parseFloat(style[`border${capitalizedSecondaryProperties[0]}Width`])
  const borderWidthEndValue = parseFloat(style[`border${capitalizedSecondaryProperties[1]}Width`])
  return {
    delay,
    duration,
    easing,
