...
<Popover class="flex md:hidden" theme="transparent">
	<div
	  ...
	  slot="tooltip">
		<div
		  ...
		  on:click={_ => (details = !details)}>
			...
			<Anchor button circle class="text-accent" tall><i class="fa fa-info text-sm" /></Anchor>
		</div>
		<div
		  class="relative z-popover flex cursor-pointer items-center justify-end gap-2 text-neutral-100"
		  on:click={() =>
        router.at(router.at("relays").of(encodeURIComponent(url)).toString()).push({modal: true})}>
			...
			<Anchor button circle class="text-accent" tall><i class="fa fa-search text-sm" /></Anchor>
		</div>
		{#if !userRelayUrls.includes(url)}
			<div
			  class="relative z-popover flex cursor-pointer items-center justify-end gap-2 text-neutral-100"
