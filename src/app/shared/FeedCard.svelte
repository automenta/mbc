...
<FlexColumn small>
    ...
    <span class="flex items-start gap-3">
      <div>
        <Anchor class=" text-xl" on:click={loadFeed}>
          <span class:line-through={deleted} class:text-neutral-400={!feed.title}>
            {displayFeed(feed)}
          </span>
        </Anchor>
	      {#if deleted}
      ...
      <div class="flex gap-1">
		...
		<slot name="controls">
			<Anchor on:click={loadFeed} underline>Load feed</Anchor>
		</slot>
	</div>
	{#if feed.description}
