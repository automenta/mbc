...
<div>
	<AltColor
	  ...
	  class="relative flex flex-col gap-8 p-6 text-neutral-100 sm:flex-row sm:gap-4">
		<div class="flex flex-col items-center gap-4">
			<PersonCircle class="mt-1 h-32 w-32" {pubkey} />
			{#if pubkey === $session?.pubkey}
				<Anchor
				  button
				  class="w-full !bg-neutral-800 dark:!bg-white"
				  on:click={router.at("/settings/profile").open}>Edit
				</Anchor>
			{:else if $session}
				<Anchor
				  button
					...
					<div>
						<div class="flex max-w-[80%] items-center gap-2 text-xl">
							...
							<div on:click|stopPropagation>
								<Popover opts={{hideOnClick: true}} triggerType="mouseenter">
									<div slot="trigger">
										<WotScore
						...
						<div class="mt-4 break-all opacity-75">
							<span>{npub}</span>
							<CopyValueSimple class="!inline-flex pl-1" label="Npub" value={npub} />
						</div>
					</div>
	...
	<div class="bg-tinted-800-d pt-3">
		<Tabs {activeTab} {setActiveTab} {tabs}>
			<div class="flex gap-2 px-2" let:tab slot="tab">
				{toTitle(tab)}
