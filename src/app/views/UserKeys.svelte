...
<FlexColumn xl>
	<FlexColumn>
		...
		<p>
			...
			<Anchor
			  external
			  href={keypairUrl}>keypair
			</Anchor
			>
			. This allows you to fully own your account, and move to another app if needed.
		</p>
		<div>
			<CopyValue encode={nip19.npubEncode} label="Public Key" value={$session?.pubkey} />
		...
		{#if $session?.method === "nip01"}
			<div>
				...
				>Do not share this with anyone.</strong>
					Be careful about copying this into other apps - instead, consider using a
					<Anchor
					  href={nip07}
					  external>compatible browser extension
					</Anchor>
					to securely store your key.
				</small>
			</div>
		{/if}
