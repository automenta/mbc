<script lang="ts">
	import {FOLLOWS, isRelayUrl, LOCAL_RELAY_URL, normalizeRelayUrl, PROFILE, RELAYS} from "src/util"
	import {deriveEvents} from "src/store"
	import {repository, session} from "src/app"
...
<Content size="lg">
	...
	{:else}
		...
		<p>
			If you'd like to select your relays manually instead, click
			<Anchor
			  underline
			  on:click={() => openModal("custom_relay")}>here
			</Anchor
			>
			.
		</p>
	...
	{#if !showFound}
		<p>
			You can also
			<Anchor underline on:click={skip}>skip this step</Anchor>
