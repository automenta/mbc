...
<FlexColumn class="max-w-md text-center">
	...
	<p>
		...
		<Anchor href="/help/remote-signers" modal underline>What's a signer?</Anchor>
	</p>
	{#if url}
		<QRCode code={url} />
	{/if}
	<Input bind:value={input} disabled={loading} placeholder="bunker://...">
		<i class="fa fa-box" slot="before" />
	</Input>
	<div class="flex gap-2">
		<Anchor button disabled={loading} on:click={back}><i class="fa fa-arrow-left" /> Back</Anchor>
		<Anchor accent button class="flex-grow" {loading} on:click={logIn}>Continue</Anchor>
	</div>
</FlexColumn>
