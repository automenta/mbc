<script lang="ts">
	...
	import {ctx, type Emitter, last} from "src/lib"
	import {hash, now, own} from "src/signer"
	import {createEvent, DVM_REQUEST_PUBLISH_SCHEDULE, toNostrURI, type TrustedEvent} from "src/util"
	import type {Thunk, ThunkStatus, ThunkStatusByUrl} from "src/app"
	import {abortThunk, session, signer, tagPubkey} from "src/app"
	import {PublishStatus} from "src/net"
	import {DVMEvent} from "src/dvm"
		...
		<FlexColumn>
			<Field label="What do you want to say?">
				<div
				  ...
				  class:text-black={!showPreview}>
					...
					<div class:hidden={showPreview}>
						<EditorContent class="min-h-24" {editor} />
					</div>
				</div>
				<div class="flex items-center justify-end gap-2 text-neutral-200">
					...
					<span>•</span>
					<button class="cursor-pointer text-sm underline" on:click={togglePreview} type="button">
						{showPreview ? "Hide" : "Show"} Preview
					</button>
					<button class="cursor-pointer text-sm" on:click={openOptions} type="button">
						<i class="fa fa-cog" />
					</button>
				</div>
			</Field>
			<div class="flex gap-2">
				...
				<button
				  class="hover:bg-white-l  flex h-7 w-7 cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded bg-white px-6 text-xl text-black transition-all"
